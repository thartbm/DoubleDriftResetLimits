...
def meridianTrial(cfg):
  ...
  # stimulus parameters:
  internalMovement  = cfg['trialdefinitions']['internalMotion'][trialno]
  #externalMovement  = cfg['trialdefinitions']['externalMovement'][trialno]
  externalMovement  = 1./6
  
  startX            = cfg['trialdefinitions']['startX'][trialno]
  startY            = cfg['trialdefinitions']['startY'][trialno]
  fixationOffset    = cfg['trialdefinitions']['fixationOffset'][trialno]
  ...
  cfg['point'].pos = [fixationOffset * (cfg['height'] / 6.), (-1./4 * cfg['height'])]
  ...
  startPos = [startX * cfg['height']/6., startY * cfg['height']/6.]
  
  # gabor will be oriented parallel to the movement direction implied by the starting position:
  # so that the internal motion will be orthogonal to that external motion
  cfg['gabor'].ori = (-1 * sp.arctan2(startPos[1],startPos[0]) * 180 / sp.pi) + 90 # psychopy works different from actual trig
  ...
  cfg['instruction'].pos = [cfg['point'].pos[0], (-1*cfg['point'].pos[1])+80]
  ...
  while (time.time() - starttime < trialduration):
    ...
    p = (1 - (abs((((elapsed)*(externalMovement/0.5))%2) - 1) * 2)) * (cfg['height'] / 6.)
