...
We've minimized depencies on other packages, but there are two that help. Of course, you can skip the chunks that use them, or just install them by uncommenting the second line of this chunk:
...
In order to determine the point of reset automatically, we took the first sign change of the X coordinate. Since the start of the tracing often was somewhat "shaky" we disregarded sign changes in the first XXX cm. Since the trajectories are measured in discrete pixel values, sign changes were clearer after slight smoothing of the trajectories, using splines.
