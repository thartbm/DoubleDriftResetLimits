...
retrace$value <- 1
retrace$value[which(is.na(retrace$resetX))] <- 0
resetcounts <- aggregate(value ~ participant, data=retrace, FUN=sum)
resetcounts$value <- resetcounts$value / (6*6*4)
print(resetcounts)
#print(mean(retrace$value))
```

We can also look at this, further split by condition:

```{r}
resetcountsCOND <- aggregate(value ~ participant + abs(internalspeed) + externalspeed, data=retrace, FUN=sum)
