...
In Figure 4 (left), we show the trajectories for participant 4 in the condition with 3 _cps_ internal motion and 4 _s_ passes. This exemplary participant's re-traced paths show clear 'jump' resets, all of which are detected by our simple algorithm. Other participants are not that clear cut.

```{r fig4, echo=FALSE, message=FALSE, fig.width=8, fig.height=3.5, fig.cap="\\label{fig:fig4}**Figure 4. Data in Experiment 2.** **Left:*** One exemplary participant's trajectories with very clear 'jump' resets at the purple dots. (Purple lines: trajectories up to resets, gray lines: trajectories after resets, blue lines: trajectories without resets.) Other participants' data was not this clear. **Middle:** Average participant reset points with standard deviations. (Gray arrow: real trajectory, blue line: spatial average, red line: temporal average.) **Right:** Illusion strength as in Figure 3; the acrtangent of the slope (internal velocity over external velocity). Gray dahsed line: unity, black line: data/model as in Cavanagh & Tse (2019) at 81% of unity, purple line: best fit here at 42% of unity."}
#plotExampleData()
plotData()
```

On average, the algorithm detects resets in ~75% of trials. For three participants, only about half the trials show resets (42%, 44% and 58%). For the remaining six participants the algorithm detects resets in 72%-99% of trials, with an average of 88%. The minimum number of resets in each of the six conditions for any participant is 7 (out of 24 trials), with the average at ~17. Since participants experiencing a "hit-the-wall reset" (as opposed to a "jump" reset) might not always display a direction change in their paths, 50% of trials with resets seems reasonable. Either type of perceived resets appears valid, and the average reset positions still indicates some limit to the illusion. Therefore, we keep all participants and resets (reset points in Figure 4, middle).

We compare the strength of the illusion as measured with half-way points (see Fig 4, right), with an earlier study [@Cavanagh2019]. A line through the origin with slope 0.49 predicts illusion strength fairly well in this data, which is weaker than earlier (0.81) or with online tracking (0.72). We can also see that illusion strength measured with re-tracing is more variable. While the illusion is weaker, it is nevertheless there and depends on the relationship between internal and external speed, as expected.

## Single Limits on the Double-Drift Illusion

Now we look at the distribution of reset points, and whether they can be explained by a temporal or spatial limit. In Figure 4 (middle), we can see that the reset points vary more along the Y-axis than the X-axis. That is, predicting reset points from a single Y coordinate (time-cutoff) would leave larger residuals than predicting reset points from a single X coordinate (or spatial cutoff). In fact, the standard deviation of reset point X-coordinates is about 21.9% of the standard deviation of reset point Y-coordinates. This already indicates that a spatial limit explains about 4 times more of the variance in reset points than a temporal limit.

```{r fig5, echo=FALSE, message=FALSE, fig.width=8, fig.height=3, fig.cap="\\label{fig:fig5}**Figure 5. Reset Models** **Left:** X-coordinates predicted from a single temporal limit. **Middle:** Y-coordinates predicted from a single spatial limit. **Right:** X- and Y-coordinates predicted from a temporal and a spatial limit."}
#plotResetPoints()
plotModels()
```
...
Where $L_y$ and $L_x$ are a temporal (y) and spatial (x) limit that are free parameters, and s corresponds to the calculated slope. If we fit these simple models by minimizing the mean square error (as the absolute distance in centimeters between predicted and actual reset points), the optimal parameters indicate a temporal limit of ~1.4 _s_ (AIC=-167.1) and a spatial limit of ~1.8 _cm_ (AIC=-220.7), as illustrated in Figure 6 (left and middle). This is close to the mean values of the respective coordinates (Figure 5, middle). The model with the spatial limit fits the data better than the model with a temporal limit ( _p_<.001, relative log-likelihood based on AICs). This again indicates that a spatial limit explains more of the data.
