...
The data clearly show the illusion, but does tracking decrease the strength of the illusion compared to simply observing it? In Figure 2 (bottom right), we compare mean illusion strength to that reported previously [@Heller2021]. As can be seen the averages from our data here (red circles, purple line) coincide fairly well with the illusion strength predicted by the model fitted to all their data (Eq. 1, in black). That is; tracking the illusion while observing it does not change the strength of the illusion.
...
If the perceived path of the gabor is approximated by a straight line (up to a reset), and reset points occur at a set limit, then a spatial limit ($L_x$) would equal the x-coordinate and predict the y-coordinate of the first reset, $y$:
