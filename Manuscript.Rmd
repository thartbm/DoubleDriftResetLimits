...
In this experiment, two tasks were performed by two separate groups of participants. Both tasks asked participants to use a stylus on a drawing tablet to track a moving gabor with some amount of internal motion that elicited the double-drift illusion. We test if tracking the illusion continuously, prevents or decreases the illusion.
...
In a second task, we only used the higher internal speed (±3 _cps_) and increased the duration of a single pass of the stimulus from 2 _s_ to 4 _s_, both to increase the occurence of spontaneous resets. Average headings and heading distributions for 6 participants can be seen in Figure 3. Despite lower external motion speed, the illusion is as strong as in the conditions with the same internal motion in the first task.
...
The illusion is clearly there in both tasks, but does tracking decrease the strength of the illusion compared to simply observing it? We compare illusion strength to that reported previously [@Cavanagh2019] in Figure 4. As can be seen the averages from our data here (in red) coincide almost exactly with the illusion strength predicted by the model. That is; tracking the illusion while observing it does not change the strength of the illusion. 

```{r fig4, echo=FALSE, message=FALSE, fig.width=8, fig.height=5, fig.cap="\\label{fig:fig4}**Figure 4. Illusion strength.** The strength of the illusion in degrees angle deviation from the physical path is plotted over external motion speed ( _dva/s_ ). In red are the average illusions strengths for ±3 _cps_, ±1 _cps and 0 cps. The black circles and line are the data and model from Cavanagh & Tse (2019). The grey lines are the illusion strengths their model predicts for the internal motion speeds used here."}
...
Figure 8 shows both the actual reset point coordinates (dots), given the illusion strength as the angle to a sample half-way to the reset point, as well as the model's predictions of reset point coordinates (lines).

```{r fig8, echo=FALSE, message=FALSE, fig.width=6, fig.height=6, fig.cap="\\label{fig:fig8}**Figure 8. Reset Points and Combined Limits**"}
plotSequentialModel()
```
...
While here we find that a spatial limit is the better explanation for resets, other evidence may point at a temporal limit instead. In an as of yet unpublished study [Sirui Liu] obtained the point of subjective equivalence (PSE) of a double drift stimulus, and a static point that appeared some time after the double drift stimulus disappeared. The double-drift stimulus was on a 2 or 4 DVA length trajectory, traversed in 1, 2 or 3 seconds. The PSE's can be modelled very accurately with a model that assumes a fixed chance of a reset per unit of time (~0.7%/s), that is: it explains the data using only a temporal bound on the illusion. Interestingly, this fully contradicts the current findings. The model predicts that the PSE decreases over time as it becomes more likely that a reset has occurred. We will compare the two data sets here (see Fig 9).

```{r fig9, echo=FALSE, message=FALSE, fig.width=6, fig.height=8, fig.cap="\\label{fig:fig9}**Figure 9. PSE's of double-drift position and continuous tracking**"}
plotAverageTrajectories()
```

If the PSE's express the average perceived deviations from the actual trajectory of the double-drift stimulus, then they should show a similar pattern as the averaged trajectories in experiment 2. In order to be able to compare the two data sets, we will include all trajectories, including those where our simple algorithm did not detect a reset, as such trials should also be included in the PSE data. Further, we scale the trajectories on both axes, so that the Y end-point matches the duration of stimulus. Since the PSE data was collected in degrees visual angle, we convert the unit to degrees visual angles, using the estimate at the centre of the workspace. For the 4-second stimuli, we removed the last second. Figure 9 shows the individual participants' and the averaged trajectories (purple, red, orange) as well as the PSE's obtained in the other study (shown separately for 2 and 4 DVA stimulus trajectories) in gray.
