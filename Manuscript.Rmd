...
```{r fig2, echo=FALSE, message=FALSE, fig.width=8, fig.height=5, fig.cap="\\label{fig:fig2}**Figure 2:** Online Tracking Data. **Left:** Individual trajectories. Note that there are only a few obvious discontinuities (highlighted in red). In general, the traced paths showed rough linearity. **Top right:** Average heading of the middle 1 _s_ of each pass. Distance from origin is time, angle is the heading angle. Dark blue lines the average heading, in light blue a polar heatmap with the distribution of heading. **Bottom right:** The strength of the illusion in degrees angle deviation from the physical path over unity slope (internal / external velocity). In red are the average illusions strengths for 0 _cps_, ±1 _cps_ and ±3 _cps_, and in light blue the data for the 4 individual participants. Gray dashed line: rediction of vector combination model of Equation 1 with $K=1$. Black line: fit with $K=0.81$ as in Cavanagh & Tse (2019), $K=0.81$. Purple: best fit to our data, $K=0.72$."}
plotBoundedTracking()
```
...
We compare the strength of the illusion as measured with half-way points (see Fig 4, right), with an earlier study [@Cavanagh2019]. A line through the origin with slope 0.49 ($K$ from Eq. 1) predicts illusion strength fairly well in this data, which is weaker than earlier ($K$=0.81) or with online tracking ($K$=0.72). We can also see that illusion strength measured with re-tracing is more variable. While the illusion is less strong and more variable, it is nevertheless there and depends on the relationship between internal and external speed as in previous studies.

## Limits on the Double-Drift Illusion

Now we look at the distribution of reset points, and whether they can be explained by a temporal or spatial limit. In Figure 4 (middle), we can see that the reset points vary more in when they occur, than how far they are from the real path of the stimulus. That is, predicting reset points from a single Y coordinate (time limit) would leave larger residuals than predicting reset points from a single X coordinate (or spatial limit). This already indicates that a spatial limit explains more of the variance in reset points than a temporal limit.

```{r fig5, echo=FALSE, message=FALSE, fig.width=8, fig.height=3, fig.cap="\\label{fig:fig5}**Figure 5:** Reset Models. **Left:** Distance of reset from true path predicted from a single temporal limit. **Middle:** Time of reset predicted from a single spatial limit. **Right:** Both spatial offset and time of resets predicted from a temporal and a spatial limit."}
#plotResetPoints()
plotModels()
```

We test this with three simple models, based on straighforward trigonometry. First, we calculate the slope of a line ($s$) through the origin and a sample halfway to the reset point, indicating the strength of the illusion. If the perceived path of the gabor is approximated by a straight line (up to a reset), and reset points occur at a set limit, then a temporal limit ($L_y$) would predict spatial offset from the physical path at the time of the reset, $x$:

[2]  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  $x = L_y / s$

And a spatial limit ($L_x$) would predict time until the first reset, $y$:

[3]  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  $y = L_x \cdot s$

While resets relying on both limits would use these two equations:

[4a]  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  $x = L_x + L_y / s$

[4b]  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  $y = L_x \cdot s + L_y$

Where $L_y$ and $L_x$ are a temporal and spatial limit that are free parameters, and $s$ corresponds to the calculated slope. If we fit these simple models by minimizing the mean square error (as the absolute distance in centimeters between predicted and actual reset points), the optimal parameters for the single limit models indicate a temporal limit of ~1.4 _s_ (AIC=-26.2) and a spatial limit of ~1.8 _cm_ (AIC=-35.1), as illustrated in Figure 6 (left and middle). This is close to the mean values of the respective coordinates of reset points (Fig. 5, middle).  The model with two limits uses a temporal limit of 0.33 _s_ and a spatial limit of 1.47 _cm_ (AIC=-34.5). The model with a single spatial limit fits the data better than the model with a temporal limit ( _p_=.01, relative log-likelihood based on AICs) but is not better than the model with combined limits ( _p_=.73). This indicates that a spatial limit explains most of the data.
...
Recently, Kwon, Tadin, and Knill [-@Kwon2015] proposed a Bayesian object tracking model to explain the double-drift illusion. In their model, when the position signals are weak, the motion signals bias perceived position away from the true location. In their case, this deviation saturated after about 200 _ms_ and the perceived path then continued parallel to the physical path, as in the “hit the wall” resets we described above [@Kwon2015, Fig. 3]. In contrast to their model, our participants in Experiment 2 showed quite variable timing for the reset point where the accumulation of offset ended. The mean time of these reset points was not 200 _ms_ but 1.5 _s_ and, more importantly, ranged from 0.5 _s_ to almost 4 _s_ (see Fig. 4). Moreover, 4/9 of our participants regularly reported “jump resets” that the Kwon et al. [-@Kwon2015] model cannot predict. The jump resets suggest that both the physical and perceived locations are available to the visual system and that when the deviation exceeds some limit, the perceived location returns toward the physical location. At that point, the build-up of the illusion may start again.

Recently, it has been suggested that resets can be triggered by a distraction of attention [@Nakayama2020]. The participants in this experiment also retraced the perceived path and reported "jump" resets. Nakayama and Holcombe also suggest that planned eye movements can trigger a “jump” reset back to the physical path. In our experiment, we did not control for eye-movements, so we can't directly investigate whether eye movements accompanied the jump resets that participants reported. However, "jump" resets occured at a fairly consistent location, which suggests that they are triggered by the stimulus and not by attentional distraction or eye movements.
