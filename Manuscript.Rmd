...
We test this with three simple models, based on straighforward trigonometry. First, we calculate the slope of a line ($s$, time/illusory offset) through the origin and a sample halfway to the reset point, indicating the strength of the illusion. If the perceived path of the gabor is approximated by a straight line (up to a reset), and reset points occur at a set limit, then a temporal limit ($L_y$) would predict spatial offset from the physical path at the time of the reset, $x$:
...
Where $L_y$ and $L_x$ are a temporal and spatial limit that are free parameters, and $s$ corresponds to the calculated slope from the start of the trajectory to the halfway point. If we fit these simple models by minimizing the mean square error (errors are the absolute distance in centimeters between predicted and actual reset points), the optimal parameters for the single limit models indicate a temporal limit of ~1.82 _s_ (Eq. 2; AIC=-20.3) and a spatial limit of ~2.46 _cm_ (Eq. 3; AIC=-28.4), as illustrated in Figure 6 (left and middle). This is close to, but not the same as, the mean values of the respective coordinates of reset points (Fig. 5, middle). The model with two limits uses a temporal limit of 0.55 _s_ and a spatial limit of 1.89 _cm_ (Eq. 4a,b; AIC=-28.8). The model with a single spatial limit fits the data better than the model with a temporal limit ( _p_=.016, relative log-likelihood based on AICs) but is not better than the model with combined limits ( _p_=.87). This indicates that a spatial limit explains most of the data.
