...
The strength of the double-drift illusion probably arises because of the very poor location information for a gabor pattern in the periphery when it has the same mean luminance as the background [@Gurnsey2012; @Kwon2015; @Cavanagh2019]. With poor positional certainty, the motion information contributes to the location estimate, generating a shift in perceived position. If the gabor itself is not moving [@DeValois1991], the shift saturates quickly [40 _ms_: @Chung2007; 100 _ms_: @Arnold2007], perhaps because the accumulating evidence of actual location of the stationary gabor patch is sufficient to anchor its position. However, when the gabor itself also moves as in the double-drift stimulus, there is no stable location information to anchor the position estimate and the build-up continues away from its physical location up to several times the size of the gabor. The strength of the illusion, or how much the perceived position shifts from the real position over time, depends on the speed of both the internal and external motion [@Tse2006; @Shapiro2010; @Kwon2015]. In our first experiment, participants track their percept with a stylus as the gabor moves back and forth on its trajectory (Fig. 1) with 4 internal speeds. We analyze the angle of the tracking to determine if manual response is susceptible to the illusion.

At some point, the illusion stops accumulating. The position may just remain at some fixed offset following a path parallel to the true path or the position may reset back toward its physical location and then accumulation begins again (Fig. 4, left) or it saturates. This long duration before resetting or saturation is well beyond the integration period of other visual effects and into the range of short-term memory. Given that both true and illusory positions are available, it could be that instead of some integration period, it is the interplay between the two position signals that leads to spontaneous resets. Alternatively, there is a simple periodicity or a stochastic process that underlies the changes in perception. Recent work has shown that independent of the stimulus, movement-related attention may lead to resets [@Nakayama2020]. This means that participants' responses to the stimulus may change the illusion. Here we will first test if real-time as well as delayed, manual tracking of the gabor's position, prevents or decreases the illusory shift in position.

The ability to record trajectories the perceived position will then allow us to study spontaneous resets – the adjustment of perceived location when the illusion has deviated too much or too long from the physical location. In particular, if a conflict between two location signals [@Lisi2015] leads to resets this would predict a set distance between the real and perceived position where resets occur. Under this hypothesis, the timing of spontaneous resets would depend on the speed of internal motion (given constant external motion). On the other hand, if spontaeous resets are the result of a process that integrates over time, resets would occur after a certain amount of time, but not be dependent on internal motion. Investigating the time and location of resets in the recorded trajectories, allows us to distinguish between a spatial limit to the illusion or a temporal limit. This in turn will inform us on the processes underlying the perception of positions of moving objects.

# Online tracking experiment
...
For reporting degrees of visual angle that stimuli spanned ( _dva_ ), we assumed the participants to be close to the setup, with their eyes at the height of the monitor. While there was no requirement to use the forehead rest, this is a reasonable assumption, since stimuli are only visible when relatively close to the setup. Nevertless, reported _dva's_ are estimates of the size of stimuli.
...
The motion of the envelope was restricted to the central 13.5 _cm_ ( ~14.0 _dva_ ) and was set so that the stimulus would move from one end to the other of its path ( a motion we call a 'pass' ) in 2, 3 or 4 _s_, corresponding to speeds of ~6.75, ~4.5 and ~3.75 _cm/s_ ( or roughly 7.0, 4.7 or 3.5 _dva/s_ ). For simplicity, and since the distance moved is always the same, these speeds will be refered to by the duration of a single pass (2, 3 or 4 _s_).
...
We tried detecting resets, but none could be unambiguously identified (see Fig 2). It is possible that resets were perceived but were too quick to respond to in the continuous tracking task. Alternatively, tracking may have stabilized the illusion, preventing resets from occurring.

```{r fig2, echo=FALSE, message=FALSE, fig.width=4.5, fig.height=6, fig.cap="\\label{fig:fig2}**Figure 2. Heading and Trajectories in Experiment 1.** **Top:** Average heading across the middle 1 _s_ of each pass. Distance from origin is time, angle is angle. In darker blue lines the average heading across participants, in light blue a polar heatmap giving an impression of the distribution of heading. **Bottom:** All individual trajectores. _Red_: Highlighted trajectories from conditions with non-zero internal speed, where at least some heading samples fell in the 95% confidence interval for that participants' heading distribution in zero internal speed condition. This would occur during a reset, but only very few trajectories have this feature, and none contain clear resets."}
plotBoundedTracking()
```
...
```{r fig3, echo=FALSE, message=FALSE, fig.width=5, fig.height=5, fig.cap="\\label{fig:fig3}**Figure 3. Illusion strength.** The strength of the illusion in degrees angle deviation from the physical path is plotted over external motion speed ( _dva/s_ ). In red are the average illusions strengths for 0 _cps_, ±1 _cps_ and ±3 _cps_, and in light blue the data for individual participants. The gray lines are the predicted illusion strengths based on the model from Cavanagh & Tse (2019) for the internal motion speeds used here. *the second panel only for testing*"}
plotStrengthComparison()
```

# Delayed retracing experiment

In a second experiment, aimed at collecting resets, we ask participants to re-trace the perceived path of a double-drift stimulus, immediately after viewing the stimulus, a method used by Nakayama & Holcombe [-@Nakayama2020]. This allows participants more time to carefully reproduce the path. We use the recorded trajectories to assess points where resets occurred. The reset points can then be used to determine if resets are time-limited or space-limited.

```{r fig4b, echo=F, fig.height=3.2, fig.width=5.62, message=FALSE, error=FALSE, warning=FALSE, fig.cap="**Figure 4: Resets.** _Left_: At some point, the perceived position will stop drifting further and further away from its actual path. For some people this takes the form of a ‘jump’ back toward the true position, for others it means the illusory position remains at a fixed offset moving parallel to the real path, and there are some in-between versions. _Top right_: if these resets are due to a spatial limit on the size of the illusory position shift, the points of the resets should have a constant X coordinate. _Bottom right_: if they occur after some period of time, they should have a constant Y coordinate."}
library(magick)
...
The setup was the same as used in the online tracking experiment, except that we now also used a small keypad, for responses (see Fig 1, right). Stimuli were identical to those used in the online tracking experiment, but now always started at the near end of the workspace and moved away from the participants. The double drift stimuli would use either 3 or 4 seconds to move 13.5 _cm_ and had an internal drift of 2, 3 or 4 _cps_.
...
We use the same heading measure here as in experiment 1. However, the individual trajectories reveal more points that are potential spontaneous resets that we try to extract. Using a smooth spline interpolated version of the trajectory we detect left-right direction changes. Since the initial movement is often jittery, we only take direction changes further than ~0.39 cm away from the starting point. The first (if any) of the remaining direction changes, are then taken as reset points. By averaging the x and y coordinates of reset points in all 9 participants and 6 stimulus conditions, we obtain 54 average reset points for further analyses and modelling. The average line orientation is also calculated for each of those 54 combinations of partcipants and stimulus conditions.
