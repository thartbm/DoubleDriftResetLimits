...
As before, we compare the strength of the illusion (see Fig 5, left), with an earlier study [@Cavanagh2019]. A line through the origin with slope 0.51 ($K$ from Eq. 1) predicts illusion strength fairly well in these data, which is weaker than the $K$=0.81 found by Cavanagh & Tse [-@Cavanagh2019] and lower than the $K$=0.72 found in Experiment 1 with online tracking. While the illusion is less strong, it is nevertheless there and depends on the relationship between internal and external speed as in previous studies.
...
```{r fig5, echo=FALSE, message=FALSE, fig.width=7.5, fig.height=4, fig.cap="\\label{fig:fig5}**Figure 5:** Illusion strength and reset models. **Left:** Illusion strength as in Figure 2 (bottom right); plotted against predicted strength from Eq.1: the arctangent of the ratio of internal to external velocities. Purple dots are the group means from 3 s passes, red from 4 s passes (2, 3 and 4 _cps_ internal speeds). Predictions from Eq. 1: Gray dashed line for $K=1$, black line; $K=0.81$, purple line; best fit here at $K=0.51$ and the shaded area shows a 95% confidence interval. **Right:** Model limits shown here come from a weighted bootstrapping across participants, lines are median parameter values, shaded areas the 95% confidence intervals of the limits. Gray dots are the reset points from all participants. While a spatial limit provides the best fit, neither a spatial nor a temporal limit explains the data well."}
#plotResetPoints()
#plotModels()
...
We test this with a series of simple models. First, for every reset point (with coordinates $X$ and $Y$) we use the illusion strength, expressed as either an angle (Fig 4, left: $a = atan2(X,Y)$) or a slope ($s = \frac{sin(a)}{cos(a)}$) to predict reset points.
