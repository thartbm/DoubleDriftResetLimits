...
For this experiment, 9 participants were recruited from the lab (6 female; ages 19 - 27, mean: 22.8). All participants reported themsleves as right handed and had either normal or corrected-to-normal vision. Procedures were in accordance with the Declaration of Helsinki (2003) and were approved by York’s Human Participants Review Committee. All participants provided prior, written, informed consent.
...
We examined 4 simple models. If there is a spatial offset limit that triggers resets, the reset points should fall on, or be close to a line with a constant x coordinate (blue line in Fig. 5A). If there is a temporal limit, all resets should have the same or a similar reset time (distance from origin / speed, the red arcs in Fig. 5A). If we assume thaat these spatial or temporal limits have some random variation from trial to trial, we can fit a normal distribution (with 2 parameters: mean and standard deviation) on both the x coordinates and the time points of resets. However, neither the x coordinates, nor the time points of resets were normally distributed (both _p_<.001, Kolmogorov-Smirnov tests of data against optimal normal distribution) suggesting that there is no hard space or time limit that triggers resets. We then used gamma distributions to capture any processes where the resets happen at random time in a trial or a random illusory offset. So we additionally fit gamma distributions (with 2 parameters: rate and shape) on the X coordinates and time points of resets as well (see Fig 5A, marginal distributions; Fig 5B,C). We then calculate the likelihoods of these four simple distribution models both using all resets and within individual participants and use this to calculate an AIC to determine a relative probability to compare the models.

```{r fig5, echo=FALSE, message=FALSE, fig.width=6, fig.height=5.333333, fig.cap="\\label{fig:fig5}**Figure 5:** Reset models. **A:** Model limits relative to reset point locations. Reset points neither fall along a spatial limit (blue line) or a temporal limit (red lines: continuous for 4 _s_ passes, dashed for 3 _s_ passes). Marginal distributions show that reset points time and spatial offset approximate a gamma distribution. **B,C:** Color illustrates the gamma probability distributions, where white is zero probability, and the darkest red or blue is the maximum probability density. This is a relatively scale-free unit, where the total surface of the full probability density function is 1. The data and likelihood distributions match fairly well. **B:** Gamma distribution on time (log likelihood: -766.6). **C:** Gamma distribution on spatial offset (log likelihood: -1319.3)."}
invisible(capture.output(plotModels()))
```

For the space and time limit with normal distributions (means in Fig 5A, blue: spatial, red: time), the time-based distribution fits better for the group data (normal space AIC=2,834.83, normal time _AIC_=1,677.98; _p_<.001) as well as all individual participants, and the same holds within gamma distribution models of reset models (gamma space AIC=2,642.69, gamma time AIC=1,537.10; _p_<.001). Furthermore, the gamma-based distribution on time of resets performed better than the normal distribution of reset time in the pooled data ( _p_<.001 ), and this holds for most participants (6/9) as well. This means that time-based distributions explain our reset points better than models assuming a spatial offset, so that the time-based gamma distribution was the most successful of all 4.

# Discussion

We first tested whether manual tracking of a double-drift stimulus is susceptible to the illusion as reported by perceptual judgments, pointing [@Lisi2017], and memory saccades [@Massendari2018; @Ueda2018]. The results of this online tracking experiment showed a close agreement between the angle of the manual tracking and the illusion strength expected based on the model from earlier perceptual measures [@Heller2021]. However, we were also interested in spontaneous resets and only ~4% of trials in this first experiment had deviations that could have been a reset. In contrast, in the second experiment, 63.8% of trials showed a reset. There was a reset within 1 _s_ in 22.3% of trials, and within 2 _s_ in 52.2% of trials. We can use these results to determine the rate of spontaneous resets that would have been expected in the first experiment. If each pass is independent and the perceived trajectories were equivalent in the two experiments, this would mean ~98.5% of trials of Experiment 1 should have shown at least 1 spontaneous reset which is far larger than the ~4% of trials we observed ( _p_<.001, exact binomial test). We are not sure why so few spontaneous resets occurred in the first experiment. Possible explanations could be either that online tracking requires relatively fast hand movements, leaving little time for drawing the details of any spontaneous resets or the demands of the online tracking increased the attention to the double-drift stimulus and with less chance of attentional distraction, the likelihood of resets was reduced [@Nakayama2020]. The one feature that sets this task apart from others is that the tracking is done while perceiving the stimulus, such that action could affect perception. One possible mechanism is that predicted sensory consequences of the hand movement influence perception ("I will move my hand in this direction, so I expect to see motion in that direction as well"). And another is post-dictive inference ("my hand has moved in this direction so I must have seen the visual stimulus move in the same direction"). But this is a question for future research.

In our second experiment, we asked participants to draw the perceived path of the stimulus after observing a single pass. The orientation of the drawn traces reflect the strength of the illusion, and in a majority of trials (63.8%) there was a clearly indentifiable reset point, of either a "return" or a "hit-the-wall" type. We then set out to test if these spontaneous resets were triggered by a limiting distance of drift away from the physical location, or by a specific limit of time. Neither of these provided a viable explanation of the pattern of resets. Resets points are broadly distributed throughout the workspace, suggesting that resets are not linked to illusion strength or stimulus onset, but may caused by a more "random" process.

We observe that reset times appear randomly sampled from a broad gamma distribution. In part, this may rely on how (un)suitable hand trajectories are as an accurate measure of reset points, as they may contain an unknown amount of noise. Therefore, we compare our results with those from an as of yet unpublished work [pre-print: @Liu2021]. This study probed how spontaneous resets might affect the accumulated illusion throughout time (1, 2 or 3 _s_) while keeping the actual path (2 or 4 _dva_) and maximum illusion constant (45° deviation, so 1.4 or 2.8 _dva_ offsets, respectively). Spontaneous resets should decrease the accumulated illusion (see Fig 6A). After the main stimulus presentation, there is a 350 _ms_ gap, during which the double-drift stimulus is not shown, and this is know to evoke a reset. After this gap-evoked reset the illusion resumes accumulating from the veridical position where the gabor appears after the gap [@Lisi2015]. By changing the location where the double-drift reappears and asking participants if it jumped left or right, the point of subjective equivalence that expresses the size of the gap-evoked jump reset can be determined.

The data from 10 participants showed that for a fixed path and illusion strength, longer durations produced smaller illusory offsets at the end (Fig 6B, round dots). This can be explained by (among others) resets randomly occuring throughout time. Liu et al. [-@Liu2021] present a Poisson process model with a constant chance of reset occurring every 10 ms of ~0.007 or ~0.7 / _s_ (Fig 6B, light diamonds). If we compare this to the gamma distribution fit, with a rate of $2.81$ events per second, and reset occurring at the $3.74^{th}$ event (on average), this amounts to a ~0.75 (2.81/3.74) chance of a reset per second. If we simply take the parameters of the gamma distribution fit to reset times in the re-tracing experiment (without optimization), we can also predict PSEs (Fig 6B, dark diamonds). We see that the two findings, based on very different approaches, are mostly in agreement with each other.
