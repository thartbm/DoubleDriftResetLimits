...
```{r fig4b, echo=F, fig.height=3.2, fig.width=5.62, message=FALSE, error=FALSE, warning=FALSE, fig.cap="**Figure 4: Resets.** **Left:** At some point, the perceived position will stop moving further away from its actual path. For some people this takes the form of a ‘jump’ back toward the true position, for others it means the illusory position remains at a fixed offset moving parallel to the real path, or some combination of the two. **Top right:** if these resets are due to a spatial limit on the size of the illusory position shift away from the true vertical path, the locations of the resets should have a constant X coordinate. **Bottom right:** if they occur instead after some period of time, they should have a constant Y coordinate."}
library(magick)
...
Now we look at the distribution of reset points, and whether they can be explained by a temporal or spatial limit. In Figure 6 (left), we can see that the reset points vary more along the Y-axis than the X-axis. That is, predicting reset points from a single Y coordinate (time-cutoff) would leave larger residuals than predicting reset points from a single X coordinate (or spatial cutoff). In fact, the standard deviation of reset point X-coordinates is about 21.9% of the standard deviation of reset point Y-coordinates. This already indicates that a spatial limit explains about 4 times more of the variance in reset points than a temporal limit.
