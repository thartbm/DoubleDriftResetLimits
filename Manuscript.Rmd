---
title: "Measuring the double-drift illusion with hand trajectories"
author: 
...
Here we use hand movements to investigate where or when the double-drift illusion resets. Eye and hand movement are differentially sensitive to many illusions, such as the Ponzo illusion [@Gamble2017], Ebbinghaus illusion [@Knol2017], flash-lag effect [@Blohm2003], the Duncker illusion [@Soechting2001] and many others. Similarly, it has been shown that the double-drift illusion affects pointing movements, but not saccades [@Lisi2017], and hand movements have been used to test other perceptual phenomena [@PatricioDecima2022]. Hence, we expect that hand trajectories will be an effective method to capture the full illusory trajectory, rather than a point estimate along the perceived path, and this will allow capturing resets of the double-drift illusion.

The strength of the illusory percept; the deviation of the perceived path of the gabor from its true path, depends on the speed of both the internal and external motion [@Tse2006; @Shapiro2010; @Kwon2015; @Cavanagh2019; @Heller2021]. We want to test how well a continuous trajectory reflects the percept of the illusion. We assess this in our first experiment, using hand movement trajectories recorded during stimulus presentation (Fig. 1). We compare trajectories across 5 different internal motions of the gabor and compare the illusion strength with previous work based on a straightforward vector combination model [@Heller2021]. As we expected, online tracking provides a similar measure of illusion strength as pointing. However, only few possible resets are apparent in the data (>4% trials), and no limit to the illusion is clear.

In our second experiment, we therefore slow down the external stimulus motion, and have participants reproduce the perceived trajectory after stimulus presentation. Here we do observe spontaneous resets, which allows us to investigate any spatial or temporal limits of the illusion. However, we find that there is no hard temporal or spatial limit leading to resets, but it seems that resets occur randomly over time, based on a gamma distribution.
...
The individual trajectories reveal some points that are potential spontaneous resets that we try to extract. We created a heuristic to extract likely reset points based on right-left inflections in the trajectories. First, we remove the first and last 4 mm of the trajectory, as well as any short segments on the trajectory at the start and end that go toward the participant (i.e. more than 180 degrees away from the true external motion), to remove any unintended jitters caused by starting and ending the movement. Then, we apply a bi-directional low-pass Butterworth filter with a cut-off frequency of 1.5 Hz. However, since participants draw at very different speeds, we first linearly interpolate the trajectories such that the sampling rate is normalized to 30 Hz for all trajectories, based on the stimulus duration. In these summary trajectories we detect the first inflection point using the X coordinates. This could put the reset point too late for hit-the-wall resets, so that we move back to get the previous sample at 95% of the X coordinate at the inflection point. Then, we locate the sample in the raw trajectory that is closest to this point. Finally, we remove a few reset points that are detected very close to the veridical path without any build-up of the illusion by only keeping reset points that fell 5 mm within the quadrant where we expect them. This heuristic will result in some misses and some false alarms, and while changing the algorithm may improve performance, there is no ground truth to evaluate this. All raw trajectories and the detected reset points can be found on the OSF repository.
...
Within the normal distribution models of reset points (means in Fig 5A, blue: spatial, red: time), the time-based distribution fits better for the group data ( _p_<.001 ) as well as all individual participants, and the same holds within gamma distribution models of reset models (Fig 5B,C). Furthermore, the gamma-based distribution on time of resets performed better than the normal distribution of reset time in the pooled data ( _p_<.001 ), and this holds for most participants (6/9) as well. This means that time-based distributions explain our reset points better than models assuming a spatial offset.

# Discussion

We first tested whether manual tracking of a double-drift stimulus is susceptible to the illusion as reported by perceptual judgments, pointing [@Lisi2017], and memory saccades [@Massendari2018; @Ueda2018]. The results of this online tracking experiment show a close agreement between the angle of the manual tracking and the illusion strength expected based on the model from earlier perceptual measures [@Heller2021]. However, we were also interested in  spontaneous resets and only ~4% of trials show a deviation in the trajectory that might reflect a reset. In the second experiment, 62% of trials showed a reset, and 36% of those occurred within 1 _s_ and 84% within 2 _s_. Based on these numbers one might expect spontaneous resets in at least 22.5% of each of the two half passes in the online tracking experiment and 52.5% of each of the full 2 _s_ passes - if each pass is independent. This would mean ~86% of trials would show at least 1 spontaneous reset which is much larger than the ~4% of trials we observe ( _p_<.001, exact binomial test). The question remains as to why so few spontaneous resets occurred in this experiment. Simple explanations could be that duration of stimulation presentation requires relatively fast hand movements, leaving little time for drawing spontaneous resets or reduced attention for the double-drift stimulus. The one feature that sets this task apart from others is that the tracking is done while perceiving the stimulus, such that action could affect perception. One possible mechanism is that predicted sensory consequences of the hand movement inform perception ("I will move my hand in this direction, so I expect to see motion in that direction as well"). And another is post-dictive inference ("my hand has moved in this direction so I must have seen the visual stimulus move in the same direction"). But this is a question for future research.

In our second experiment, we asked participants to draw the perceived path of the stimulus after observing a single pass. The orientation of the drawn traces reflect the strength of the illusion, and in a majority of trials (62%) there was a clearly indentifiable reset point, for either a "return" reset or a "hit-the-wall" reset. We then set out to test if these spontaneous resets are triggered by a limiting distance of drift away from the physical location, or by a specific limit of time. A gamma distribution of reset time seems to explain this data best.

However, it does not seem like there is a specific temporal limit on time, i.e. resets do not occur at a given time after stimulus onset, but appear randomly sampled from a wide distribution. In part, this may rely on the hand trajectories we recorded here to provide an accurate measure of reset points, as they may contain an unknown amount of noise. Therefore, we compare our results with those from an as of yet unpublished work [pre-print: @Liu2021]. This study used a psychophysics-based approach to probe how spontaneous resets might affect the accumulated illusion throughout time (1, 2 or 3 _s_) while keeping the actual path (2 or 4 _dva_) and maximum illusion (1.4 or 2.8 _dva_, respectively) constant. Spontaneous resets should decrease the accumulated illusion (see Fig 6A). After the main stimulus presentation, there is a 350 _ms_ gap, during which the double-drift stimulus is not shown, and this is know to evoke a reset. After this gap-evoked reset the illusion resumes accumulating from the veridical position where the gabor appears after the gap [@Lisi2015]. By changing the location where the double-drift reappears and asking participants if it jumped left or right, the point of subjective equivalence that expresses the size of the gap-evoked jump reset can be determined.

The data from 10 participants shows that as a stimulus is presented for longer, the illusory offset at the end gets smaller (Fig 6B, round dots). This can be explained by (among others) resets randomly occuring throughout time. Liu et al. [-@Liu2021] present a Poisson process model with a constant chance of reset occurring every 10 ms of ~0.007 or ~0.7 / _s_ (Fig 6B, light diamonds). If we compare this to the gamma distribution fit, with a rate of $2.81$ events per second, and reset occurring at the $3.74^{th}$ event (on average), this amounts to a ~0.75 (2.81/3.74) chance of a reset per second. If we simply take the parameters of the gamma distribution fit to reset times in the re-tracing experiment (without optimization), we can also predict PSEs (Fig 6B, dark diamonds). We see that the two findings, based on very different approaches, are mostly in agreement with each other.

```{r fig6, echo=FALSE, message=FALSE, fig.width=6, fig.height=4, fig.cap="\\label{fig:fig6}**Figure 6:** Spontaneous resets without hand trajectories. Experiment and Results based on data from Liu et al. [-@Liu2021], re-analyzed here. **A:** By ensuring the perceived path of a double-drift stimulus is straight down (teal arrows), any gap-induced reset, should be equal to the accumulated illusion if there are no spontaneous resets (black arrow), but smaller if there are (light blue arrow). Gaps were presented after 1, 2 or 3 seconds, and 2 or 4 dva real paths, which allows psychophysics to quantify the amount of illusion left after several periods of abserving a double-dirft stimulus. **B:** Points of subject equivalence show that the size of the illusion decreases with time even as the real path, and maximum illusion is kept constant. These data are in line with resets occurring randomly over time. The simplest model presented by Liu et al. (2021) keeps the illusion strength constant and fits a chance of reset per time unit as ~0.007 / 10 ms to match the gap-evoked reset (diamond symbols)."}
plotJumpPSEs(target = 'inline', individualDots = FALSE, modelDots = TRUE, gammaDots=TRUE)
```

Kwon, Tadin, and Knill [-@Kwon2015] proposed a Bayesian object tracking model to explain the double-drift illusion and other phenomena. In their model, when the position signals are weak, the motion signals bias perceived position away from the true location. Their model predicts that this deviation saturates after about 200 _ms_ and the perceived path then continues parallel to the physical path, as in the “hit the wall” resets we describe above [@Kwon2015, Fig. 3]. In contrast to their model, the reset in our re-tracing task show quite variable timing, and do not show a clear spatial offset (Fig. 5A) but seem to fall on a fairly wide gamma distribution on reset time (Fig. 5B), which is at odds with a set limit (either spatial or temporal) where resets occur. This seems to argue against a perceptual mechanism underlying resets, such as on going Bayesian inference until the discrepancy between retinal and perceived position signals is too large. In any case, the best fitting temporal limit of these reset points was not 200 _ms_ but 1.3 _s_ and 99% of detected reset times are above 200 _ms_ (see Fig. 5A). Moreover, 4/9 of our participants clearly reported “return” resets that the Kwon et al. [-@Kwon2015] model does not predict, and only 1/9 participants showed clear "hit-the-wall" resets. The “return” resets suggest that both the physical and perceived locations are available to the visual system and that when spontaneous resets occur, the perceived location returns toward the physical location. While our results speak against a particular, stimulus or perception induced mechanism leading to resets, the illusion itself may still have limits, and the way resets are perceived may relate to stimulus properties as well as to illusory perception. Regardless, future models that aim to predict perception in double-drift stimuli need to also predict resets.

Recently, it has been suggested that resets can be triggered by a distraction of attention [@Nakayama2020]. The participants in this experiment also retraced the perceived path and reported “return” resets. Nakayama and Holcombe [-@Nakayama2020] suggest that planned eye movements can trigger a “return” reset back to the physical path. In our experiment, we did not control for eye-movements, so we can't directly investigate whether eye movements accompanied the “return” resets that participants reported. However, the random distribution of resets throughout time are in line with a mechanism based on distracted attention, in some cases leading to eye-movement planning.

We find that manual tracking of the double-drift stimulus during its motion did show the expected illusion, although not always as strong. Few if any resets were detected during online tracking, perhaps because of the additional attentional requirements or the short duration (2 s), although interactions with the simultaneous hand movements could explain this too. Participants did report spontaneous resets in delayed, single-pass re-tracing of Experiment 2. The resets were best explained by a gamma distribution on reset time. This finding seems to indicate that spontaneous resets occur independently from the stimulus or how it is perceived, and spontaneous reset may even be random. If there is a mechanism causing resets we have not uncovered it here, and if resets are independent from perception, the illusory percept itself may still be limited, and give rise to how resets are perceived.
